...
This directory contains the **VoltrActionProvider** implementation, which provides actions for voltr operations.

## Overview

The VoltrActionProvider is designed to work with SvmWalletProvider for blockchain interactions. It provides a set of actions that enable [describe the main purpose/functionality].

## Directory Structure

```
voltr/
├── voltrActionProvider.ts       # Main provider implementation
├── voltrActionProvider.test.ts  # Provider test suite
...
2. Implement the action in `voltrActionProvider.ts`:
   ```typescript
   @CreateAction({
     name: "new_action",
