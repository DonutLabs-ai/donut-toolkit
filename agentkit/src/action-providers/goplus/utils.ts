 ...
 */
export function createBatchSummary(results: ProcessedTokenSecurity[]): BatchTokenSecurityResult['summary'] {
  return {
    safeTokens: results.filter(r => r.securityScore >= SECURITY_THRESHOLDS.SAFE).length,
    riskyTokens: results.filter(r => 
      r.securityScore >= SECURITY_THRESHOLDS.RISKY && 
      r.securityScore < SECURITY_THRESHOLDS.SAFE
    ).length,
