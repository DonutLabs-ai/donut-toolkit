...
export class SNSAPI {
  ...
  async resolveDomain(domain: string): Promise<string | null> {
    try {
      ...
      throw new Error(`Failed to resolve domain: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  ...
  async reverseLookup(address: string): Promise<string | null> {
    try {
      ...
      throw new Error(`Failed to perform reverse lookup: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  ...
  async getDomainInfo(domain: string): Promise<any> {
    try {
      ...
      throw new Error(`Failed to get domain info: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  ...
  async searchDomains(query: string, limit: number = 10): Promise<any[]> {
    try {
      ...
      throw new Error(`Failed to search domains: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
