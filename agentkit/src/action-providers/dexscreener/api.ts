...
export class DexScreenerAPI {
  ...
  async searchToken(query: string): Promise<any> {
    ...
    try {
      ...
      throw new Error(`Failed to search tokens: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  ...
  async getTokenPairs(address: string): Promise<any> {
    ...
    try {
      ...
      throw new Error(`Failed to get token pairs: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  ...
  async getPairInfo(pairAddress: string): Promise<any> {
    ...
    try {
      ...
      throw new Error(`Failed to get pair info: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
} 