...
This directory contains the **SolayerActionProvider** implementation, which provides actions for solayer operations.

## Overview

The SolayerActionProvider is designed to work with SvmWalletProvider for blockchain interactions. It provides a set of actions that enable [describe the main purpose/functionality].

## Directory Structure

```
solayer/
├── solayerActionProvider.ts       # Main provider implementation
├── solayerActionProvider.test.ts  # Provider test suite
...
2. Implement the action in `solayerActionProvider.ts`:
   ```typescript
   @CreateAction({
     name: "new_action",
